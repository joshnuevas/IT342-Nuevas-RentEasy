    ...
    <Page>
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-7 rounded-lg bg-white p-6 shadow-sm ring-1 ring-stone-200">
          <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#d5673f]">My profile</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight text-stone-950">Account and rental history</h1>
        </div>

        <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
          <aside className="h-fit rounded-lg border border-stone-200 bg-white p-6 text-center shadow-sm">
            <div className="mx-auto grid h-24 w-24 place-items-center rounded-full bg-[#2f513f] text-3xl font-black text-white">
              {userInitials(profile.email)}
            </div>
            <h2 className="mt-4 text-xl font-black text-stone-950">{profile.name}</h2>
            <p className="text-sm text-stone-500">{profile.email}</p>
            <div className="mt-6 space-y-2 text-left text-sm font-bold text-stone-600">
          ...
          <div className="space-y-6">
            <form onSubmit={handleSubmit} className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between">
                <h2 className="text-lg font-black text-stone-950">Profile information</h2>
                {saved && <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-[#2f513f]">Saved</span>}
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-11 items-center gap-2 rounded-lg bg-[#2f513f] px-5 font-black text-white">
            ...
            <section className="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
              <h2 className="mb-5 flex items-center gap-2 text-lg font-black text-stone-950">
                <History className="h-5 w-5 text-[#d5673f]" />
                Order history
              </h2>
...
function ProfileNav({ icon: Icon, label }) {
  return (
    <div className="flex items-center gap-3 rounded-lg bg-stone-50 px-4 py-3">
      {createElement(Icon, { className: "h-4 w-4 text-[#d5673f]" })}
      {label}
    </div>
  );
}
